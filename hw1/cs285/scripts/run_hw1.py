    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                for path in paths:
                    # path["observation"] is (T, ob_dim); expert returns (T, ac_dim)
                    with torch.no_grad():
                        expert_acs = expert_policy.get_action(path["observation"])
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          ob_batch, ac_batch,_,_,_ = replay_buffer.sample_random_data(params['train_batch_size']) 
